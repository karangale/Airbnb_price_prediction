...
def splitZip(zipcode):
    ...
    '''
    This function removes the special characters present in the zip codes.

    Params:
        zipcode: (alphanumeric) The zipcode which needs to be processed to conform to a standard format
    Returns:
...
def clean_airbnb(airbnb_data_req):
    ...
    '''
    This function is for cleaning up the airbnb data.
    It handles multiple use cases like null handling, imputing empty cell data etc.
...
def rest_add(row_data):
    '''
    This function applies a row level transformation to the restaurant data to return the restaurant name.
...
def clean_restraunts_geo(restaurant_data):
    '''
    This function limits the geography of the data to around New York.
    ...
    restaurant_data = restaurant_data[restaurant_data['Longitude']>LONGITUDE_LOWER]
    restaurant_data = restaurant_data[restaurant_data['Longitude']<LONGITUDE_UPPER]
